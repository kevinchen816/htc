...
class CamerasController extends Controller
{
    ...
    /*----------------------------------------------------------------------------------*/
    public function Action_Search($camera_id, $action_code) {
    ...
    public function Action_Add($camera_id, $action_code) {
        $action = new Action;
        $action->camera_id = $camera_id;
        $action->action = $action_code;
        $action->status = 1; //'Requested';
        ...
        if ($request_id) {
            ...
            if ($action) {
                if (($action->camera_id == $param['camera_id']) &&
                    ...
                    ($action->status == 1)) {
                    $data['status'] = 2; // 1:requested, 2:completed, 3:cancelled, 4:failed
    ...
    */
        ...
        */
        ...
        if ($err == 0) {
            if ($request->Arm == 'Y') {
                ...
                if ($this->Action_Search($camera->id, 'PS') == 0) {
    ...
    public function uploadthumbX(Request $request, ImageUploadHandler $uploader) {
        //return $request;
...
/*
{
    ...
    "ActionCode": "UO",
    "ParameterList": {
        "FILENAME": "PICT0062.JPG",
        "IMAGESIZE": "5",
        "COMPRESSION": "28",
        "REQUESTID": "17853"
    },
    "DateTimeStamp": "2018-09-29 04:00:14"
}
*/
